...
function get_wholesale_orders($user_id, $record_count, $page, $where = '', $order = '', $pagesize = 10) 
{
	...
	$arr = array();
	$sql = 'SELECT og.ru_id,og.goods_attr ,og.goods_number, oi.inv_type, oi.inv_payee, oi.tax_id, oi.postscript, oi.main_order_id, oi.consignee, oi.order_id,oi.mobile, oi.order_sn, oi.order_status, oi.add_time, ' . '(oi.order_amount) AS total_fee, ' . 'og.goods_id, oi.email, oi.address, oi.province, oi.city, oi.district ' . ' FROM ' . $GLOBALS['ecs']->table('wholesale_order_info') . ' as oi' . ' left join ' . $GLOBALS['ecs']->table('wholesale_order_goods') . ' as og on oi.order_id = og.order_id' . $left_join . ' WHERE oi.user_id = \'' . $user_id . '\' and oi.is_delete = 0 ' . $where . ' and (select count(*) from ' . $GLOBALS['ecs']->table('wholesale_order_info') . ' as oi2 where oi2.main_order_id = oi.order_id) = 0 ' . 'group by oi.order_id ORDER BY oi.add_time DESC ' . $limit;
	$res = $GLOBALS['db']->query($sql);
	while ($row = $GLOBALS['db']->fetchRow($res)) 
	{
...
function wholesale_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true) 
{
	...
	if ($res === NULL) 
	{
		...
		if ($data === false) 
		{
			$sql = 'SELECT w.cat_id, w.cat_name, w.parent_id, w.sort_order, COUNT(s.parent_id) AS has_children ' . 'FROM ' . $GLOBALS['ecs']->table('wholesale_cat') . ' AS w ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('wholesale_cat') . ' AS s ON s.parent_id = w.cat_id ' . 'GROUP BY w.cat_id ' . 'ORDER BY w.parent_id, w.sort_order ASC';
			$res = $GLOBALS['db']->getAll($sql);
			if (count($res) <= 1000) 
			{
