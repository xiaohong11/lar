	...
	public function actionUrl()
	{
		if (IS_POST) {
			...
			if ($type == 'function') {
				...
				foreach ($url as $key => $value) {
					$url[$key] = array('cat_id' => $value['topic_id'], 'cat_name' => $value['name'], 'parent_id' => 0, 'start_time' => $value['start_time'], 'end_time' => $value['end_time'], 'topic_img' => $value['topic_img'], 'url' => url('topic/index/detail', array('topic_id' => $value['topic_id'])));
				}
		...
		if ($authority == 1) {
			...
			if ($_FILES['file']) {
				...
				if ($res['error'] == 0) {
					...
					$img_thumb = str_replace(dirname(ROOT_PATH) . '/', '', $img_thumb);
