      ...
      if (confirm(res.message))  
        {
          ...
          location.href = ROOT_URL + 'index.php?m=flow&a=mgaddress';
        }
        else{
...
region.cccDdd = function(district_id, city_id, user_id, d_null){
    ...
    var province_id = document.getElementById('province_id').value;
     var url = ROOT_URL + 'index.php?m=flow&a=add_address/in_stock';
    if( city_id <= 0){
        return false;
    }
    if(d_null == 1){
   ...
   $.post(ROOT_URL + 'index.php?m=flow&a=show_region_name',{province:province,city:city,district:district},function(obj){
