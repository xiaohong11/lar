...
(function($){
	...
	$.addFlex = function(t,p)
	{
        ...
        if (p.title)
        {
            ...
            xDiv.innerHTML = '<div class="ftitle"><h3>'+p.title+'</h3><h5></h5></div>';
            if (p.reload) {
                xDiv.innerHTML += ' <div class="pReload" title="刷新数据"><i class="icon icon-refresh"></i></div><div class="refresh_span">刷新 - 共{$record_count}条记录</div><div class="sline"></div>';
            }
		...
		if (p.columnControl && $('th',g.hDiv).length)
		{
			...
			.html('<span class="hBtn"><i class="icon icon-cog"></i></span>')
			.hover ( function () {
			 	$(g.nDiv).show(); 
